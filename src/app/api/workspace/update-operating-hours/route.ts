...
// POST: Update operating hours for a workspace
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if user is an owner (only owners can update operating hours)
    if (workspaceMember.role !== "owner") {
      return NextResponse.json(
        { error: "Only workspace owners can update operating hours" },
        { status: 403 }
      );
    }
